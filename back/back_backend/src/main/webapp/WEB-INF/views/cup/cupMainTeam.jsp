      ...
      <div class="round body">
        ...
        <div class="tabnum" style="margin-bottom:20px;">
          <c:forEach var="i" begin="1" end="${cupInfoMap.group_count }" step="1">
        	<a class="btn-large gray-o w6 btn-tab" data-id="num${i}">${i}조</a>
          </c:forEach>
        </div>
		...
		<input name="excelFlag" type="hidden" value="cupMainTeam">
		<input name="ageGroup" type="hidden" value="${ageGroup}">
          ...
          <tbody>
            <tr>
              ...
              <td class="tr">
				...
				<a class="btn-large default" onclick="gotoAddExcel();" >본선 참가팀 일괄 등록</a>
...
<!--         <div class="show-area"  id="num1"> -->
        ...
        <form name="frm" id="frm" method="post"  action="save_cupMainTeam" onsubmit="return false;">
		...
		<c:set var="teamRow" value="${fn:split(teamRowData, '|')}"/>
          ...
          <colgroup>
            ...
            <c:forEach var="i" begin="1" step="1" end="${cupInfoMap.main_group_count}">
            <col width="*">
            </c:forEach>
			...
			<c:forEach var="team_count" begin="0" step="1" end="${cupInfoMap.m_team_count - 1}" varStatus="status"> <!-- ROW 조별 팀수 ex)4팀-->
				<tr id="tr${status.index}">
					...
					<c:forEach var="group_count" begin="0" step="1" end="${cupInfoMap.m_group_count-1}"> <!-- CELL 조 갯수 -->
						<c:choose>
				  		<c:when test="${empty cupMainTeamList}">
			  				<td id="td${tdNum }" class="input2">
				  				<select class="w50">
				                  <option>3조</option>
				                </select>
				  		...
				  		<c:when test="${fn:length(cupMainTeamList) <= cnt }">
		  					<c:set var="cnt" value="${cnt + 1 }" />
		  					<c:set var="tdNum" value="${tdNum + 1 }" />
				  			
				  			<td id="td${tdNum }" class="input2">
									<select id="selGroup${tdNum}" class="w50">
									<option value="-1">조선택</option>
				  					<c:forEach var="_group_count" begin="1" step="1" end="${cupInfoMap.group_count}">
					                  <option value="${_group_count}">${_group_count}조</option>
				  					</c:forEach>
				                </select>
				                <select id="selTeam${tdNum}" class="w50">
									
				                	<option value="-1">순위선택</option>
				  					<c:forEach var="_team_count" begin="1" step="1" end="${cupInfoMap.team_count}">
					                  <option value="${_team_count}">${_team_count}위</option>
				  					</c:forEach>
				                </select>
				  			</td>
				  		</c:when>
					...
					<td class="admin">
		              <a><i class="xi-plus-circle-o"></i></a>
		  ...
		  <!-- 
			...
			<tr>
			  ...
			  <tr>
				<td>
					<div class="w65 fl"><input type="text"></div>
        ...
        <div class="body-foot">
          <div class="others">
          	<a class="btn-large default" onclick="gotoDelCupMainTeam(2);">본선 참가팀 일괄 삭제</a>
          	
          	<c:choose>
          	<c:when test="${fn:length(cupTeamList) eq 0 }">
	            <a class="btn-large default" onclick="gotoAddLeagueTeam(0);">본선 참가팀 등록</a>
	            <a class="btn-large gray-o" onclick="gotoAddLeagueMatch(0);">본선 참가팀 등록 후 일정등록</a>
          	</c:when>
          	<c:otherwise>
	            <a class="btn-large default" onclick="gotoAddLeagueTeam(1);">본선 참가팀 수정</a>
...
<!-- 임시이동 -->
	            ...
	            <a class="btn-large gray-o" onclick="gotoAddCupMainMatch('${cupInfoMap.cup_id}');">본선 일정등록 이동</a>
				</c:if>
...
<form name="delFrm" id="delFrm" method="post"  action="save_cupMainTeam" onsubmit="return false;">
