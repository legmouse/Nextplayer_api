...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			/* <c:if test="${not empty cupSubMatchRank}">
				<c:forEach var="i" begin="0" end="${fn:length(cupSubMatchRank) - 2}">
					<c:forEach var="j" begin="0" end="${fn:length(cupSubMatchRank) - 1}">
						<c:if test="${i ne j}">
							<c:if test="${cupSubMatchRank[i].rankPoint eq cupSubMatchRank[j].rankPoint}">
								$('#' + '${cupSubMatchRank[i].team_id}').css('border', '1px solid red');
								$('#' + '${cupSubMatchRank[j].team_id}').css('border', '1px solid red');
								console.log(1);
							</c:if>
						</c:if>
					</c:forEach>
	...
	if(!isEmpty(teamSearch)){
		...
		//대회 예선 영상 등록/수정
		function gotoCupSubMatchVideo(cmIdx, index){

			var live = $("[name='live"+index+"']").val();
			var rep = $("[name='rep"+index+"']").val();
			var high = $("[name='high"+index+"']").val();
			console.log('--- gotoCupSubMatchVideo cmIdx :'+ cmIdx +', index : '+ index + ', live :'+ live +', rep : '+ rep  +', high : '+ high);

			$("#videofrm [name='cupSubMatchId']").val(cmIdx);
		...
		//대회 예선 최종순위 확정
		function gotoAddCupSubRank(){
			if (confirm("예선 "+"${groups}"+"조 최종순위를 확정 하시겠습니까?")) {
				$('input[name=sFlag]').val('0');
				document.rankfrm.submit();
			}
		}
		//대회 예선 최종순위 수정
		function gotoFixCupSubRank(){
			if (confirm("예선 "+"${groups}"+"조 최종순위를 수정 하시겠습니까?")) {
				$('input[name=sFlag]').val('6');
				document.rankfrm.submit();
			}
		}
		//대회 예선 최종순위 삭제
		function gotoDelCupSubRank(){
			if (confirm("예선 "+"${groups}"+"조 최종순위를 삭제 하시겠습니까?")) {
		...
		//리그 경기일정 정보 이동
		function gotoCupMgrMatch(groups) {
			var url = 'cupMgr';
	    	var ageGroup = "${ageGroup}";

			$('input[name=groups]').val(groups);
			document.cmgrmfrm.submit();
		}
		...
		const formatDateString = (dateString) => {
			const date = new Date(
					dateString.replace(/[^0-9]/g, '').replace(/^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})$/, '$1-$2-$3T$4:$5'),
			)
		...
		const getWordForGameProgress = (matchDate) => {
			let WordForProgress = ''
			const currentDate = new Date()
			const MatchDate = new Date(formatDateString(matchDate))

			const dateNum = (MatchDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60)
			if (dateNum > 9) WordForProgress = '예정'
			else if (dateNum < 7) WordForProgress = '종료'
			else WordForProgress = '진행중'
			return WordForProgress
		}
		...
		const goDetailLineup = (matchId, ageGroup) => {
			const cupId = '${cupId}';
			location.href = '/cupMgrSubMatchPlayData?cupId=' + cupId + '&matchId=' + matchId + '&ageGroup=' + ageGroup;
		}
		
		//대회 경기 개별 수정
		function gotoSaveCupSubMatchOne(idx, match_id){
			...
			const selEnd = $('select[name="selEnd' + idx + '"] option:selected').val();
			...
			$('input[name="cupSubMatchId"]').val(match_id);
			$('input[name="date"]').val(date);
			$('input[name="time"]').val(time);
			$('input[name="place"]').val(place);
			$('input[name="selHome"]').val(selHome);
			$('input[name="selAway"]').val(selAway);
			$('input[name="homeNickName"]').val(selHomeStr);
			$('input[name="awayNickName"]').val(selAwayStr);
			$('input[name="endFlag"]').val(selEnd);
			document.subMatchFrm.submit();
		}
		...
		const getAllMatchScore = (date) => {
			...
			const matchSize = "${fn:length(cupSubMatchResultList)}";
		...
		<div class="round body">
				...
				<table cellspacing="0" class="update">
					...
					<tr>
						...
						<td class="tr">
							...
							<a class="btn-large default" onclick="gotoAddExcel();" >
								<c:choose>
									<c:when test="${cupInfoMap.cup_type eq '4'}">
										${teamType}차 풀리그 일정 일괄 등록
									</c:when>
			...
			<div id="num1">
				<div class="tabnum" style="margin-bottom:20px;">
					<c:choose>
						<c:when test="${ageGroup == 'U12' or ageGroup == 'U11'}">
							<c:choose>
								<c:when test="${teamType == '1' or teamType == ''}">
									<c:forEach var="i" begin="1" end="${cupInfoMap.group_count }" step="1">
										<a class="btn-large w6 btn-tab" data-id="num${i}" onclick="gotoCupMgrMatch(${i});">${i}조</a>
									</c:forEach>
								</c:when>
								<c:when test="${teamType == '2'}">
									<c:forEach var="i" begin="1" end="${cupInfoMap.m_group_count }" step="1">
										<a class="btn-large w6 btn-tab" data-id="num${i}" onclick="gotoCupMgrMatch(${i});">${i}조</a>
									</c:forEach>
								</c:when>
								<c:otherwise>
				...
				<div class="scroll">
					<form name="frm" id="frm" method="post"  action="save_cupMgrSubMatch" onsubmit="return false;">
						<input type="hidden" name="sFlag" value="6">
						<input type="hidden" name="mvFlag">
						<input type="hidden" name="groups" value="${groups}">
						<input type="hidden" name="ageGroup"  value="${ageGroup}">
						<input type="hidden" name="trCnt" value="${fn:length(cupSubMatchResultList)}">
						...
						<table id="frmTB"cellspacing="0" class="update fixed over">
							...
							<tbody id="tblmId">
							<c:if test="${empty cupSubMatchResultList}">
								<tr class="item0">
									<td id="idEmptyList" colspan="17">등록된 내용이 없습니다.</td>
								</tr>
							</c:if>

							<c:forEach var="result" items="${cupSubMatchResultList}" varStatus="status">
								<tr class="item${status.index}">
									<td>
											${result.playdate}
										<input type="hidden" name="cupSubMatchId${status.index}" value="${result.cup_sub_match_id}">
									</td>
									<td>${result.ptime}</td>
									...
									<td>
										<input type="text" class="tc" <c:if test="${result.upd_flag eq 1 or result.upd_flag eq 2}">style="border: 1px solid red;"</c:if> name="homeScore${status.index}" value="${result.home_score }" placeholder="홈팀 점수 입력" autocomplete="off" >
									</td>
									<td>
									...
									<td class="tl">
										...
										<a class="title" onclick="gotoTeamMgrDet('${result.away_id}');">${result.away}
										<c:choose>
									...
									<td class="tc">
										...
										<%-- <td><input type="text" name="reason${status.index}" value="${result.reason}" placeholder="사유 입력">
                                            <c:choose>
									...
									<td class="admin" style="font-size: 14px;">
										...
										<div class="table-layer" id="layer${status.index}-1" style="width: 500px;">
											<table cellspacing="0" class="update">
												...
												<tbody>
												<tr>
													<td><input type="text" name="live${status.index}" placeholder="생중계 주소" autocomplete="off" value="${result.video_live}"></td>
													...
													<td class="admin">
														<a class="btn-large default" onclick="gotoCupSubMatchVideo('${result.cup_sub_match_id}', '${status.index}');">수정</a>
										...
										<div class="table-layer" id="layer${status.index}-2">
											<table cellspacing="0" class="update">
												...
												<tr>
													...
													<td>
														<select class="w100" id="selEnd${status.index}" name="selEnd${status.index}">
															<option value="0" <c:if test="${result.end_flag eq '0'}">selected</c:if>>진행</option>
															<option value="1" <c:if test="${result.end_flag eq '1'}">selected</c:if>>종료</option>
														</select>
													</td>
													<td class="admin">
														<a class="btn-large default" onclick="gotoSaveCupSubMatchOne('${status.index}', '${result.cup_sub_match_id}')">수정</a>
														<a class="btn-large gray-o layer-close">취소</a>
													</td>
												</tr>
												</tbody>
									...
									<td>
										...
										<c:choose>
											<c:when test="${result.time_diff > 0 && result.end_flag eq 0}">
												<a class="btn-large default fixBtn" onclick="fnEndMatch(${result.cup_sub_match_id}, '${result.home}', '${result.away}', '${result.home_id}', '${result.away_id}', '${pushEnd.type_id}', '${pushEnd.case_title}', '${pushEnd.case_text}')">경기종료</a>
											</c:when>
									...
									<td>
										<c:choose>
											<c:when test="${result.match_status eq 'READY' }">
												<a class="btn-large default" onclick="getPlayDataUp(this, '${status.index}', 'LINEUP', '${result.cup_sub_match_id}','${result.match_order}', '${cupInfoMap.cup_name_origin}', '${ageGroup}', '${result.home_id}', '${result.away_id}', '${result.home}', '${result.away}');">불러오기</a>
											</c:when>
											<c:when test="${result.match_status eq 'LINEUP' }">
												<a class="btn-large gray-o" onclick="getPlayDataUp(this, '${status.index}', 'LINEUP', '${result.cup_sub_match_id}','${result.match_order}', '${cupInfoMap.cup_name_origin}', '${ageGroup}', '${result.home_id}', '${result.away_id}', '${result.home}', '${result.away}');">불러오기</a>
											</c:when>
									...
									<td>
										<c:choose>
											<c:when test="${result.send_flag eq '0' and result.match_status ne 'READY'}">
												<a class="btn-large default" onclick="sendPush('LINEUP', '${result.cup_sub_match_id}', '${result.home}', '${result.away}', '${result.home_id}', '${result.away_id}', '${pushLineup.type_id}', '${pushLineup.case_title}', '${pushLineup.case_text}')">발송하기</a>
											</c:when>
											<c:otherwise>
									...
									<td>
										<c:choose>
											...
											<c:when test="${result.match_status eq 'LINEUP' }">
												<a class="btn-large default" id="playBtn_${status.index}" onclick="getPlayDataUp(this, '${status.index}', 'PLAY', '${result.cup_sub_match_id}','${result.match_order}', '${cupInfoMap.cup_name_origin}', '${ageGroup}', '${result.home_id}', '${result.away_id}', '${result.home}', '${result.away}');">불러오기</a>
											</c:when>
											<c:when test="${result.match_status eq 'START' }">
												<a class="btn-large default" id="playBtn_${status.index}" onclick="getPlayDataUp(this, '${status.index}', 'PLAY', '${result.cup_sub_match_id}','${result.match_order}', '${cupInfoMap.cup_name_origin}', '${ageGroup}', '${result.home_id}', '${result.away_id}', '${result.home}', '${result.away}');">불러오기</a>
											</c:when>
									...
									<td>
										<c:choose>
											...
											<c:when test="${result.match_status eq 'LINEUP' }">
												<a class="btn-large default" id="detailBtn_${status.index}" onclick="goDetailLineup('${result.cup_sub_match_id}', '${ageGroup}');">상세정보</a>
											</c:when>
											<c:when test="${result.match_status eq 'START' }">
												<a class="btn-large default" id="detailBtn_${status.index}" onclick="goDetailLineup('${result.cup_sub_match_id}', '${ageGroup}');">상세정보</a>
											</c:when>
					...
					<div class="others">
						<c:choose>
							<c:when test="${fn:length(cupTeamList) eq 0 }">
								<a class="btn-large default" onclick="gotoAddCupSubMatch();">예선결과 등록</a>
								<!-- <a class="btn-large gray-o" onclick="gotoAddLeagueMatch(0);">예선결과 등록 후 팀 생성</a> -->
							</c:when>
							<c:otherwise>
								<a class="btn-large default" onclick="gotoAddCupSubMatch();">예선결과 수정</a>
						...
						<c:if test="${cupInfoMap.cup_type ne 2}">
							<a class="btn-large gray-o" onclick="gotoCupTourMatchResult('${cupInfoMap.cup_id}');">토너먼트 결과 이동</a>
						</c:if>
						<a class="btn-large default" onclick="getAllMatchScore('${cupInfoMap.sdate1}');">예선결과 일괄 불러오기</a>
			...
			<form name="rankfrm" id="rankfrm" method="post"  action="save_cupMgrSubMatchRank" onsubmit="return false;">
				<input name="ageGroup" type="hidden" value="${ageGroup}">
				<input type="hidden" name="trCnt" value="${fn:length(cupSubMatchRank)}">
				<input type="hidden" name="cupId" value="${cupId}">
				<input type="hidden" name="groups" value="${groups}">
				<input type="hidden" name="sFlag" value="">
				<input type="hidden" name="cupType" value="${cupInfoMap.cup_type}">
				<input type="hidden" name="thisRound" value="예선">
				<input type="hidden" name="teamType" value="${teamType}">
				...
				<table cellspacing="0" class="update over fixed">
					...
					<c:if test="${empty cupSubMatchRank}">
						<tr class="item0">
							<td id="idEmptyList" colspan="14">등록된 내용이 없습니다.</td>
						</tr>
					</c:if>

					<c:if test="${empty cupSubMatchRankByFinal}">
						<c:forEach var="result" items="${cupSubMatchRank}" varStatus="status">
							<tr>
								<td>
								...
								<td>
									<input type="checkbox" name="resultType${status.index}" id="resultType${status.index}" value="4" <c:if test="${result.result eq '예선' }">checked</c:if>><label for="resultType${status.index}">탈락</label>
								</td>
					...
					<c:if test="${!empty cupSubMatchRankByFinal}">
						<c:forEach var="result" items="${cupSubMatchRankByFinal}" varStatus="status">
							<tr>
								<td>
									<select id="selRank${status.index}" name="selRank${status.index}">
										<c:forEach var="i" begin="1" end="${cupInfoMap.team_count }" step="1">
								...
								<td>
									<input type="checkbox" name="resultType${status.index}" id="resultType${status.index}" value="4" <c:if test="${result.result eq '예선' }">checked</c:if>><label class="fs11" for="resultType${status.index}">탈락</label>
								</td>
			...
			<div class="body-foot">
				...
				<div class="others">
					...
					<c:if test="${!empty cupSubMatchRank}">
						<c:if test="${empty cupSubMatchRankByFinal}">
							<strong style="color: red; margin-right: 20px;">승자승 원칙이 반영된 순위입니다</strong>
						</c:if>
						<c:choose>
							<c:when test="${empty cupSubMatchRankByFinal}">
								<a class="btn-large default" onclick="gotoAddCupSubRank();">최종순위 확정</a>
							</c:when>
							<c:otherwise>
								<a class="btn-large default" onclick="gotoFixCupSubRank();">최종순위 수정</a>
								<a class="btn-large default" onclick="gotoDelCupSubRank();">최종순위 삭제</a>
							</c:otherwise>
						</c:choose>
...
<form name="cmgrmfrm" id="cmgrmfrm" method="post"  action="cupMgrSubMatch">
...
<form name="videofrm" id="videofrm" method="post"  action="save_cupMgrSubMatchVideo" onsubmit="return false;">
	<input type="hidden" name="sFlag" value="1">
	<input name="ageGroup" type="hidden" value="${ageGroup}">
	<input name="cupId" type="hidden" value="${cupId}">
	<input name="groups" type="hidden" value="${groups}">
	<input name="cupSubMatchId" type="hidden" value="">
...
<form name="subMatchFrm" id="subMatchFrm" method="post" action="save_cupMgrSubMatchOne" onsubmit="return false;">
	<c:choose>
		<c:when test="${(ageGroup eq 'U12' || ageGroup eq 'U11') && (teamType ne null && teamType ne '')}">
			<input name="teamType" type="hidden" value="${teamType}">
		</c:when>
	...
	<input name="endFlag" type="hidden" value="">
</form>
