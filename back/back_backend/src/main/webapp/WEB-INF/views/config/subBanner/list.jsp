...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success : function(data) {
            detail = data.bannerConfigList;

            let str = '';

            $.each(detail, function(idx, item) {
                str += '<tr id="' + item.banner_config_id + '" data-index="' + idx + '" class="categoryInfo">';
                str += '<td>' + item.config_name + '</td>';
                str += '<td>';
                if (item.use_flag == '0') {
                ...
                str += '<input type="hidden" class="seq" value="'+ item.banner_config_id+'">';
                str += '<input type="checkbox" id="currentTr' +idx+ '" name="currentTr' +idx+ '" class="currentTr">';
                str += '</td>';
                str += '<td><button class="btn-large default" onclick="fnGoBannerDet(\'' + item.config_key + '\',\'' + item.banner_config_id + '\')">수정하기</button></td>';
    ...
    $(document).on("click", ".btn_save", function() {
        ...
        for(var i = 0; i < seq.length; i++) {
            ...
            row[i] = {
                "bannerConfigId": seq[i].value,
                "useFlag": useFlag[i].value
            };

        }

        if(confirm("현재 배너 상태를 저장하시겠습니까?")) {
            $.ajax({
                url: '/saveBannerConfig.do',
                type: 'POST',
                dataType: 'JSON',
...
const fnGoBannerDet = (key, id) => {
    let newForm = $('<form></form>');
    newForm.attr('name', 'newForm');
    newForm.attr('method', 'post');
    newForm.attr('action', '/subBannerDet');
    newForm.append($('<input/>', {type: 'hidden', name: 'bannerConfigId', value: id }));
    newForm.append($('<input/>', {type: 'hidden', name: 'configKey', value: key }));
    $(newForm).appendTo('body').submit();
}
