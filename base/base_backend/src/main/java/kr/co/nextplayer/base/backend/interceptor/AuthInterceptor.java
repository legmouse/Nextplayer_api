package kr.co.nextplayer.base.backend.interceptor;
...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    	logger.info("authInterceptor was called.");
        // session 객체를 가져옴
        ...
        //Spring 3.1이하에서는 <exclude-mapping> 대신 아래와 같이 처리
        //exclude-mapping 대신, 요청된 url
        String requestUrl = request.getRequestURL().toString();
        System.out.println("---------- requestUrl:"+requestUrl);
        //하단의 Url 체크를 통해, login 페이지는 예외처리를 해줘야 무한 리디렉션에서 벗어날 수 있다
    	if(requestUrl.contains("/login") || requestUrl.contains("/craw/getGameTitle")){
    		System.out.println("---------- login 예외처리");
    		return true;
    	}
        ...
        if ( obj ==null ){// 로그인된 세션이 없는 경우...
            ...
            if ( loginCookie !=null ){// 쿠키가 존재하는 경우(이전에 로그인때 생성된 쿠키가 존재한다는 것)
                ...
                User userVO = service.checkUserWithSessionKey(sessionId);
...
//    		response.sendRedirect("/NSCMIMS/login");
