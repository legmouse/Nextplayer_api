...
@Service
@Transactional
public class KakaoService extends CommService {

    //https://developers.kakao.com/apps/#/list 카카오 개발자 사이트
    ...
    public Auth kakaoCallback(String code, String state, String fcmToken, String error, String error_description, AuthConfig config,
                              String keepLogin,
        ...
        if (StringUtils.isNotEmpty(error)) {
            ...
            try {
                AuthKakaoRequest request = new AuthKakaoRequest(config, AuthDefaultSource.KAKAO);
    ...
    //카카오 연동/회원 가입
    public Boolean kakaoCallback_mapping_join(MemberSnsRegDto memberSnsDto,
                                              String userId, String passwd,
                                              ServerHttpRequest serverRequest) throws CommonLogicException {

        log.info("================kakaoCallback_mapping_join=============");
        //회원체크
        Member member = loginService.mapping_sns_checkUser(userId, passwd);
        //해당 회원이 이미연동되어 있는지 체크
        MemberSns memberSns = snsMapper.checkSnsExistByMemberCd("KAKAO", member.getMemberCd());
        if (memberSns != null) {
    ...
    //카카오 연동
    public Boolean kakaoCallback_mapping(String code, String state, String error, String error_description, AuthConfig config,
                                         String memberCd,
                                         ServerHttpRequest serverRequest) throws CommonLogicException {
        log.info("================kakaoCallback_mapping=============");
    ...
    protected Boolean mapping_sns(Auth auth, String memberCd, ServerHttpRequest serverRequest) throws CommonLogicException {
        ...
        //기존 연동된 회원이 있는지
        MemberSns old_memberSns = snsMapper.checkSnsExist("KAKAO", auth.getMemberSns().getSnsId());
    ...
    protected Auth parseSnsInfo(Map<String, Object> tokenMap) throws Exception {
        ...
        Phonenumber.PhoneNumber koPhoneNumber = phoneUtil.parse(getValue(resMap.get("phone_number")), "KR");
        String phone = phoneUtil.format(koPhoneNumber, PhoneNumberUtil.PhoneNumberFormat.NATIONAL);

        String birthDate = "";
        if(!StringUtils.isEmpty(getValue(resMap.get("birthday")))){
            birthDate = getValue(resMap.get("birthday")).substring(0,2) + "-" + getValue(resMap.get("birthday")).substring(2,4);
        }
        MemberSnsRegDto memberSns = MemberSnsRegDto.builder()
            .snsType("KAKAO")
            .snsId(getValue(tokenMap.get("id")))
            .name(getValue(resMap.get("name")))
            .nickname(getValue(propertiesMap.get("nickname")))
            .email(getValue(resMap.get("email")))
            .sex(getSex(resMap.get("gender")))
            .age(Integer.parseInt(getValue(resMap.get("birthyear"))))
            .ageGroup(getValue(resMap.get("age_range")))
            .birthDate(birthDate)
            .phone(phone.replaceAll("-",""))
            .photo(getValue(propertiesMap.get("profile_image")))
            .build();
