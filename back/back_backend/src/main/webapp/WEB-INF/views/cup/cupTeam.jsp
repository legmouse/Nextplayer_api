...
$(document).ready(function() {
	...
	$(".btn-pop").on("click", function() {
		const cupTeamId = $(this).attr("data-cupTeamId");
		$("#modOne").val(cupTeamId);
	});
...
function gotoDelCupTeamOne(cupTeamId, sFlag) {

	if (cupTeamId) {
		$("input[name=sFlag]").val(sFlag);
		$("input[name=cupTeamId]").val(cupTeamId);
	}

	const confirmMsg = confirm("해당 데이터를 삭제 하시겠습니까?");
...
function gotoAddCupTeam(sFlag){
	...
	const cupType = '${cupInfoMap.cup_type}'

	console.log('tBody : ', tBody);

	if (cupType != '3') {
		for (let i = 0; i < tBody[0].children.length; i ++) {
			const tr = tBody[0].children[i];
			for (let j = 0; j <tr.children.length; j++) {
				if (j > 0) {
					const team1 = {
						nick_name: tr.children[j].querySelector('input') ? tr.children[j].querySelector('input[name="nickName"]').value : '',
						team_id: tr.children[j].querySelector('input') ? tr.children[j].querySelector('input[name="teamId"]').value : '',
						groups: tr.children[j].querySelector('input') ? tr.children[j].querySelector('input[name="groups"]').value : '',
						teams: tr.children[j].querySelector('input') ? tr.children[j].querySelector('input[name="teams"]').value : '',
					};
		...
		$.ajax({
			...
			data: {
				sFlag: sFlag,
				mvFlag: $('input[name=mvFlag]').val(),
				ageGroup: $('input[name=ageGroup]').val(),
				trCnt: $('input[name=trCnt]').val(),
				cupId: $('input[name=cupId]').val(),
				cupName: $('input[name=cupName]').val(),
				cupType: $('input[name=cupType]').val(),
				teamType: $('input[name=teamType]').val(),
				teamData: JSON.stringify(data) // 배열 데이터를 문자열로 변환하여 전송
			},
	...
	//document.frm.append($('<input/>', {type: 'hidden', name: 'teamData', value: data}));
...
function gotoTeamByTarget(trId, tdId, index){
	$('#find-club').show();
	clearTeamPopup();
	
	console.log("trId:"+trId+", tdId:"+tdId+", index:"+index);
	$('#addFrm input[name=trId]').val(trId);
	$('#addFrm input[name=tdId]').val(tdId);
	$('#addFrm input[name=index]').val(index);
/*
	if(tdCnt != null || tdCnt != ''){
		$('input[name=index]').val(tdCnt);
	}
...
function addContentsHtml(teamId, nickName, teamType, trId, tdId, index){
	console.log('-- [addContentsHtml] trId :' + trId + ', tdId :' + tdId + ', index : ' + index);
	const cupTeamId = $("#modOne").val();
	resetTd(trId, tdId, index);
	...
	szHtml += nickName;
	if (index) {
		szHtml += "</a> <a class='co999' onclick='renewTd("+trId+", "+tdId+", "+ index+");'><i class='xi-close-circle-o'></i></a>";
	} else {
	...
	if (index) {
		$("#tr"+trId+" #td"+index+"").append(szHtml);
	} else {
...
function resetTd(trId, tdId, index){
	//console.log('-- [resetTd] trId :' + trId + ', tdId :' + tdId );

	if (index) {
		$("#tr"+trId+" #td"+index+"").empty();
	} else {
...
function renewTd(trId, tdId, index){
	console.log('-- [renewTd] trId :' + trId + ', tdId :' + tdId + ', index :' +index);
...
function rowAddContentsHtml(teamId, teamName, nickName, teamType, addr, emblem, index){
	...
	let cupTeamId = $("input[name=cupTeamId" + index + "]").val();
	...
	szHtml += "<td class='admin'>";
		...
		if(index == null || index == ''){
			...
			szHtml += "<input name='newTab"+trCnt+"' type='hidden' value='1'>";
			...
			szHtml += "<a onclick='removeTeam("+index+ ", " + cupTeamId + ");'><i class='xi-close-circle-o'></i></a>";
			szHtml += "<input name='cupTeamId"+index+"' type='hidden' value='"+cupTeamId+"'>";
...
let delClick = 0;
function removeTeam(index, cupTeamId){
	...
	let str = "<input type='hidden' name='delTab"+delClick+"' value=" + cupTeamId + " />"
	$(".delDiv").append(str);
	delClick++;
...
function resetRowDataOrder(){
	...
	for (var i = 0; i < trCnt; i++) {
		 ...
		 $("#frmTB > tbody tr:eq("+i+") td:eq(5) > a:eq(1)").attr('onclick', 'removeTeam(\''+i+'\', \'ddd\')');
...
const gotoCupTeamType = (cupId, uage, teamType) => {
	let newForm = $('<form></form>');
	newForm.attr('name', 'newForm');
	newForm.attr('method', 'post');
	newForm.attr('action', '/cupTeam');
	newForm.append($('<input/>', {type: 'hidden', name: 'cupId', value: cupId }));
	newForm.append($('<input/>', {type: 'hidden', name: 'ageGroup', value: uage }));
	newForm.append($('<input/>', {type: 'hidden', name: 'teamType', value: teamType }));
	$(newForm).appendTo('body').submit();
}

const gotoAddCupFullLeague = (cupId, uage, teamType) => {
	let newForm = $('<form></form>');
	newForm.attr('name', 'newForm');
	newForm.attr('method', 'post');
	newForm.attr('action', '/cupSubMatch');
	newForm.append($('<input/>', {type: 'hidden', name: 'cupId', value: cupId }));
	newForm.append($('<input/>', {type: 'hidden', name: 'ageGroup', value: uage }));
	newForm.append($('<input/>', {type: 'hidden', name: 'teamType', value: teamType }));
	$(newForm).appendTo('body').submit();
 ...
 <div class="contents active">
      ...
      <div class="round body">
		...
		<input name="cupId" type="hidden" value="${cupId}">
			<c:choose>
				<c:when test="${ageGroup eq 'U12' || ageGroup eq 'U11'}">
					<c:choose>
						<c:when test="${cupInfoMap.cup_type ne '4'}">
							<input name="teamColumn" type="hidden" value="${cupInfoMap.group_count}">
						</c:when>
						<c:when test="${cupInfoMap.cup_type eq '4' && teamType eq '1'}">
							<input name="teamColumn" type="hidden" value="${cupInfoMap.group_count}">
						</c:when>
						<c:when test="${cupInfoMap.cup_type eq '4' && teamType eq '2'}">
							<input name="teamColumn" type="hidden" value="${cupInfoMap.m_group_count}">
						</c:when>
					</c:choose>
				</c:when>
				<c:otherwise>
...
<!--         <div class="show-area"  id="num1"> -->
        ...
        <c:when test="${cupInfoMap.cup_type eq 3 }">  <!-- 토너먼트 -->
        	 <table id="frmTB"cellspacing="0" class="update">
			  ...
			  <tbody>
			  	<tbody class="teamTBody" id="tblmId">
				...
				<tr class="item${status.index}">
					...
					<td>${result.addr}</td>
					<td class="admin">
						<a class="btn-pop" data-id="find-club" onclick="gotoTeam(${status.index});"><i class="xi-plus-circle-o"></i></a>
					  <a onclick="gotoDelCupTeamOne('${result.cup_team_id}', 3);"><i class="xi-close-circle-o"></i></a>
					  <input name="cupTeamId${status.index}" type="hidden" value="${result.cup_team_id}">
        ...
        <c:otherwise>
	          ...
	          <colgroup>
	            <col width="55px">
			  	...
			  	<c:choose>
					<c:when test="${ageGroup != 'U12' and ageGroup != 'U11'}">
				  ...
				  <c:choose>
					...
					<c:otherwise>
						<c:choose>
							...
							<c:when test="${cupInfoMap.cup_type eq '4' && teamType == '1'}">
								<c:forEach var="i" begin="1" step="1" end="${cupInfoMap.group_count}">
									<th>${i}조</th>
								</c:forEach>
							</c:when>
							<c:when test="${cupInfoMap.cup_type eq '4' && teamType == '2'}">
								<c:forEach var="i" begin="1" step="1" end="${cupInfoMap.m_group_count}">
									<th>${i}조</th>
								</c:forEach>
							</c:when>
			  ...
			  <tbody class="teamTBody">
				...
				<c:set var="rowCnt" value="${cupInfoMap.group_count * cupInfoMap.team_count}"/>

				<c:choose>
					<c:when test="${(ageGroup == 'U12' or ageGroup == 'U11') and cupInfoMap.cup_type == '4'}">
						<c:choose>

							<c:when test="${teamType == '1'}">
								<c:forEach var="team_count" begin="0" step="1" end="${cupInfoMap.team_count-1 > 0 ? cupInfoMap.team_count-1 : 0}" varStatus="status"> <!-- ROW 조별 팀수 ex)4팀-->
									<tr id="tr${status.index}" class="teamTr">
										...
										<c:forEach var="group_count" begin="0" step="1" end="${cupInfoMap.group_count-1 > 0 ? cupInfoMap.group_count-1 : 0}"> <!-- CELL 조 갯수 -->
											<c:choose>
												<%--
                                                <c:when test="${empty cupTeamList}">
                                                 ...
                                                 --%>
												<c:when test="${fn:length(cupTeamList) <= cnt and empty cupTeamList}"> <!-- 리스트는 존재하지만 해당 조/자리에 팀등록이 비어있음 OR 비어있음 -->
													<c:set var="cnt" value="${cnt + 1 }" />
													<c:set var="tdNum" value="${tdNum + 1 }" />

													<td id="td${tdNum }" class="input2 teamTd">
															<%--					  				<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}');"><i class="xi-plus-circle-o"></i></a>--%>
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}', '${tdNum}');"><i class="xi-plus-circle-o"></i></a>
													</td>
												</c:when>

												<c:otherwise>
													<c:forEach var="result" items="${cupTeamList}" varStatus="status2" begin="${cnt }" end="${cnt }">
														<c:set var="cnt" value="${cnt + 1 }" />
														<c:set var="tdNum" value="${tdNum + 1 }" />

														<c:if test="${group_count+1 eq result.groups }">
															<td id="td${tdNum }" class="input2">
																<a class="btn-pop title" data-id="find-club" data-cupTeamId="${result.cup_team_id}" <c:if test="${result.team_id eq '-1'}">style="color:red;"</c:if>>
																	<c:if test="${result.team_id eq '-1'}">[매칭오류]</c:if>
																	<c:if test="${result.team_id ne '-1'}">[${result.team_type}]</c:if>

																		${result.nick_name }</a>
																<a class="co999" onclick="gotoDelCupTeamOne(${result.cup_team_id}, 3)"><i class="xi-close-circle-o"></i></a>
															</td>
														</c:if>
										...
										<c:if test="${!empty cupTeamList && status.last}">
											<c:forEach var="group_count" begin="0" step="1" end="${cupInfoMap.group_count-1}" varStatus="status3">
												<c:if test="${group_count == cupInfoMap.group_count-1
													and status.index == cupInfoMap.team_count-1
													and (cupTeamList[fn:length(cupTeamList)-1].groups ne group_count +1)}">
													<td id="td${rowCnt}" class="input2">
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}', '${rowCnt}');">
							...
							<c:when test="${teamType == '2'}">
								<c:forEach var="team_count" begin="0" step="1" end="${cupInfoMap.m_team_count-1 > 0 ? cupInfoMap.m_team_count-1 : 0}" varStatus="status"> <!-- ROW 조별 팀수 ex)4팀-->
									<tr id="tr${status.index}" class="teamTr">
										...
										<c:forEach var="group_count" begin="0" step="1" end="${cupInfoMap.m_group_count-1 > 0 ? cupInfoMap.m_group_count-1 : 0}"> <!-- CELL 조 갯수 -->
											<c:choose>
												<%--
                                                <c:when test="${empty cupTeamList}">
                                                    <td id="td${group_count + 1}" class="input2">
										...
										<c:if test="${!empty cupTeamList && status.last}">
											<c:forEach var="group_count" begin="${cupTeamList[fn:length(cupTeamList)-1].groups}" step="1" end="${cupInfoMap.m_group_count}" varStatus="status3">
												<c:if test="${(cupTeamList[fn:length(cupTeamList)-1].groups ne group_count)}">
													${cupInfoMap.m_group_count}
													<c:set var="tdNum" value="${tdNum+1}"/>
													<td id="td${tdNum}" class="input2">
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count}', '${tdNum}');">
															<i class="xi-plus-circle-o"></i>
														</a>
													</td>
												</c:if>
										...
										<%--<c:if test="${!empty cupTeamList && status.last}">
											<c:forEach var="group_count" begin="${cupTeamList[fn:length(cupTeamList)-1].groups}" step="1" end="${cupInfoMap.group_count}" varStatus="status3">
												<c:if test="${(cupTeamList[fn:length(cupTeamList)-1].groups ne group_count)}">
													<c:set var="tdNum" value="${tdNum+1}"/>
													<td id="td${tdNum}" class="input2">
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count}', '${tdNum}');">
							...
							<%--<c:otherwise>
								...
								<c:forEach var="team_count" begin="0" step="1" end="${cupInfoMap.m_team_count-1 > 0 ? cupInfoMap.m_team_count-1 : 0}" varStatus="status"> <!-- ROW 조별 팀수 ex)4팀-->
									<tr id="tr${status.index}" class="teamTr">
										...
										<c:forEach var="res1" items="${teamRow}" varStatus="index1">
											<c:if test="${index1.index == m_team_count }">
												<td>${res1} 팀</td>
											</c:if>
										</c:forEach>
										...
										<c:forEach var="group_count" begin="0" step="1" end="${cupInfoMap.m_group_count-1 > 0 ? cupInfoMap.m_group_count-1 : 0}"> <!-- CELL 조 갯수 -->
											<c:choose>
												...
												<c:when test="${fn:length(cupTeamList) <= cnt and empty cupTeamList}"> <!-- 리스트는 존재하지만 해당 조/자리에 팀등록이 비어있음 OR 비어있음 -->
													...
													<td id="td${tdNum }" class="input2 teamTd">
															&lt;%&ndash;					  				<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}');"><i class="xi-plus-circle-o"></i></a>&ndash;%&gt;
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}', '${tdNum}');"><i class="xi-plus-circle-o"></i></a>
													</td>
												</c:when>
									...
									<c:choose>
										...
										<c:otherwise>
											...
											<c:forEach var="result" items="${cupTeamList}" varStatus="status2" begin="${cnt }" end="${cnt}">
												...
												<c:if test="${group_count+1 eq result.groups }">
													<td id="td${tdNum }" class="input2">
														<a class="btn-pop title" data-id="find-club" data-cupTeamId="${result.cup_team_id}" <c:if test="${result.team_id eq '-1'}">style="color:red;" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}', '${tdNum}');"</c:if>>
															<c:if test="${result.team_id eq '-1'}">[매칭오류]</c:if>
															<c:if test="${result.team_id ne '-1'}">[${result.team_type}]</c:if>
															${result.nick_name }
														</a>
														<a class="co999" onclick="gotoDelCupTeamOne(${result.cup_team_id}, 3)">
												...
												<c:if test="${group_count+1 ne result.groups}">
												<%--<c:if test="${(cupInfoMap.group_count * cupInfoMap.team_count) > cupInfoMap.cup_team}">--%>
													<c:set var="cnt" value="${cnt - 1 }" />
													<td id="td${tdNum }" class="input2">
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}', '${tdNum}');">
															<i class="xi-plus-circle-o"></i>
														</a>
													</td>
												</c:if>

												<%--<c:if test="${group_count == cupInfoMap.group_count-1 and status.last}">
													<td id="td${tdNum}" class="input2">
														<a class="btn-pop" data-id="find-club" onclick="gotoTeamByTarget('${status.index}', '${group_count + 1}', '${tdNum}');">
															<i class="xi-plus-circle-o"></i>
														</a>
...
<!-- 임시이동 -->
				<c:if test="${cupInfoMap.cup_type ne 3 and cupInfoMap.cup_type ne 4}" >
				...
				<c:if test="${cupInfoMap.cup_type eq 4 and teamType eq 1}" >
					<a class="btn-large default" onclick="gotoCupTeamType('${cupInfoMap.cup_id}', '${ageGroup}', 2);">참가팀2 수정</a>
					<a class="btn-large default" onclick="gotoAddCupFullLeague('${cupInfoMap.cup_id}', '${ageGroup}', 1);">1차 풀리그 일정등록 이동</a>
				</c:if>
				<c:if test="${cupInfoMap.cup_type eq 4 and teamType eq 2}" >
					<a class="btn-large default" onclick="gotoCupTeamType('${cupInfoMap.cup_id}', '${ageGroup}', 1);">참가팀1 수정</a>
					<a class="btn-large default" onclick="gotoAddCupFullLeague('${cupInfoMap.cup_id}', '${ageGroup}', 2);">2차 풀리그 일정등록 이동</a>
				</c:if>
		...
		<div class="body" style="padding:15px 20px;">
		  <div class="search">
			<form id="addFrm" onsubmit="return false;">
			  ...
			  <input type="hidden" name="index">
			  <input type="hidden" id="modOne" name="cupTeamId">
...
<form name="delFrm" id="delFrm" method="post"  action="save_cupTeam" onsubmit="return false;">
   ...
   <input type="hidden" name="cupTeamId" value="">
   <input type="hidden" name="teamType" value="${teamType}">
</form>
