...
function gotoCupMgrSubMatch(cupId){
	...
	window.open("/cupMgrTourMatch?ageGroup=" + ageGroup + "&cupId=" + cupId, "_blank");
}
 ...
 <div class="contents active">
      ...
      <div class="round body">
        <div class="body-head">
          ...
          <div class="others">
          	<a class="btn-large gray-o" onclick="gotoTeamMgr();"><i class="xi-long-arrow-left"></i> [학원]/[클럽]관리 리스트</a>
          </div>
        </div>
        <br>
			  ...
			  <c:set var="sysYear"><fmt:formatDate value="${now}" pattern="yyyy" /></c:set> 	
	          	<c:forEach var="i" begin="2014" end="${sysYear}" step="1" varStatus="status">
		          	...
		          	<c:when test="${empty sYear and (sysYear - status.count + 1) eq sysYear}">
		          		<a class="btn-large default" onclick="gototeamMgrDetYear('${sysYear - status.count + 1 }');">${sysYear - stuatus.count}</a>
		          	</c:when> 
		          	<c:otherwise>
            ...
            <c:forEach var="result" items="${teamTotalMatchList}" varStatus="status">
              <div class="team-history">
                <c:choose>
               	...
               	<c:when test="${teamInfoMap.team_id ne result.home_id and teamInfoMap.pId eq result.home_id}">
               		<c:choose>
		          	<c:when test="${result.home_score > result.away_score}">
		          		<div class="result win" style="background-image: url(/NP${result.away_emblem})">
		                  <span>승</span>
               	...
               	<c:when test="${teamInfoMap.team_id ne result.away_id and teamInfoMap.pId eq result.away_id}">
               		<c:choose>
		          	<c:when test="${result.home_score > result.away_score}">
		          		<div class="result lose" style="background-image: url(/NP${result.home_emblem})">
		                  <span>패</span>
                ...
                <p class="name">
                	...
                	<c:when test="${teamInfoMap.team_id eq result.home_id}">
			          	...
			          	${result.away}
                  <c:choose>
                <c:when test="${result.away_type eq '0'}"><span class="">[학원]</span></c:when>
                <c:when test="${result.away_type eq '1'}"><span class="">[클럽]</span></c:when>
                <c:when test="${result.away_type eq '2'}"><span class="">[유스]</span></c:when>
                </c:choose>
                	</c:when>
                	<c:when test="${teamInfoMap.team_id ne result.home_id and teamInfoMap.pId eq result.home_id}">
                ...
                <c:when test="${result.away_type eq '2'}"><span class="">[유스]</span></c:when>
                	...
                	<c:when test="${teamInfoMap.team_id eq result.away_id}">
			          	...
			          	${result.home}
                  <c:choose>
                <c:when test="${result.home_type eq '0'}"><span class="">[학원]</span></c:when>
                <c:when test="${result.home_type eq '1'}"><span class="">[클럽]</span></c:when>
                <c:when test="${result.home_type eq '2'}"><span class="">[유스]</span></c:when>
                </c:choose>
                	</c:when>
                	<c:when test="${teamInfoMap.team_id ne result.away_id and teamInfoMap.pId eq result.away_id}">
