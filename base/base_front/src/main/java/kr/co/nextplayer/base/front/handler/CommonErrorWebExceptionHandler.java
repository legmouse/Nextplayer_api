package kr.co.nextplayer.base.front.handler;
...
/**
 * kr.co.nextplayer.nextshop.app.backstage.handler.CommonExceptionHandler 의 WebFlux 버전
 */
@Slf4j
@Component
@Order(-2)
public class CommonErrorWebExceptionHandler extends AbstractErrorWebExceptionHandler {
    ...
    private Mono<ServerResponse> renderErrorResponse(ServerRequest request, ErrorAttributes errorAttributes) {
        ...
        if (error instanceof CommonLogicException) {
            ...
            responseDto.setMsg(error.getMessage());
        } else if (error instanceof BaseValidateException) {
            responseDto.setError(ApiState.VALIDATE.getCode(), error.getMessage());
        }
