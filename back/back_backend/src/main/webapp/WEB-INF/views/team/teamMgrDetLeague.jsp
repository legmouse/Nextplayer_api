...
var _graph1_cate = [<c:forEach items="${teamLeagueResultByYearList}" var="result" varStatus="status">
	"${result.play_year}"
...
var _graph1_series = [{
        name: '패',
        data: [<c:forEach items="${teamLeagueResultByYearList}" var="result" varStatus="status">
		"${result.lost}"
		<c:if test="${status.last eq false}">,</c:if></c:forEach>]
      }, {
        name: '무',
...
var _graph2_cate = [<c:forEach items="${teamLeagueRankList}" var="result" varStatus="status">
	"${result.play_year}"
	<c:if test="${status.last eq false}">,</c:if></c:forEach>
];
...
var _graph2_series = [{
	 	name: '순위',
        data: [<c:forEach items="${teamLeagueRankList}" var="result" varStatus="status">
        "${result.rank}"
		<c:if test="${status.last eq false}">,</c:if></c:forEach>]
     }];
...
var _graph3_cate = [<c:forEach items="${teamLeagueAvgGoalList}" var="result" varStatus="status">
	"${result.play_year}"
	<c:if test="${status.last eq false}">,</c:if></c:forEach>
];
...
var _graph3_series = [{
        ...
        data: [<c:forEach items="${teamLeagueAvgGoalList}" var="result" varStatus="status">
		"${result.avg_gf}"
		<c:if test="${status.last eq false}">,</c:if></c:forEach>]
    },{
        name: '실점',
...
function makeChart1(){
	  var options = {
		...
		options.series = _graph1_series;
		options.xaxis.categories = _graph1_cate;
...
function makeChart3(){
	...
	options.series = _graph3_series;
	options.xaxis.categories = _graph3_cate;
...
//클럽관리 - 요약정보 - 연도 이동
function gototeamMgrDetLeagueYear(year){
	$('#tmdlfrm input[name=sYear]').val(year);
 ...
 <div class="contents active">
      ...
      <div class="round body">
        ...
        <div class="body-head" style="border-bottom: 1px solid #eee;margin-bottom: 15px">
          ...
          <div class="others">
            ...
            >
              ...
              <a class="active" onclick="gototeamMgrDetLeague();">리그정보</a>
       ...
       <div class="body-body">
			  ...
			  <c:set var="sysYear"><fmt:formatDate value="${now}" pattern="yyyy" /></c:set> 	
	          	<c:forEach var="i" begin="2014" end="${sysYear}" step="1" varStatus="status">
		          	...
		          	<c:when test="${(sysYear - status.count + 1) eq sYear}">
		          		<a class="btn-large default" onclick="gototeamMgrDetLeagueYear('${sysYear - status.count + 1 }');">${sysYear - status.count + 1 }</a>
		          	</c:when> 
		          	<c:when test="${empty sYear and (sysYear - status.count + 1) eq sysYear}">
		          		<a class="btn-large default" onclick="gototeamMgrDetLeagueYear('${sysYear - status.count + 1 }');">${sysYear - stuatus.count}</a>
		          	</c:when> 
		          	<c:otherwise>
		          		<a class="btn-large gray-o" onclick="gototeamMgrDetLeagueYear('${sysYear - status.count + 1 }');">${sysYear - status.count + 1}</a>
		          	</c:otherwise>
		          	</c:choose>
	          	</c:forEach>
          ...
          <h4>리그 승/무/패 추이
              <c:if test="${isLeagueResultDuplicate}">
                  <br />
          ...
          <c:forEach var="result" items="${teamLeagueEnterList}" varStatus="status">
	      <c:if test="${leagueId ne result.league_id}">
	      
	      	<h4>${result.league_name}</h4><br>
            <div class="scroll">
            <table cellspacing="0" class="update fixed over">
              <colgroup>
                <col width="7%">
                <col width="7%">
                  <col width="*">
              ...
              <tbody>
              	...
              	<c:forEach var="res" items="${teamLeagueEnterList}" varStatus="status">
                  <c:if test="${result.league_id eq res.league_id}">

		              <c:choose>
		              <c:when test="${teamInfoMap.pId ne 0 }">
		                <c:if test="${teamInfoMap.pId eq res.team_id}"><tr class="currTeam"></c:if>
		                <c:if test="${teamInfoMap.team_id eq res.team_id}"><tr class="currTeam"></c:if>
		              </c:when>
		              <c:otherwise>
                    ...
                    <td>${res.nick_name}</td>
                    <td><c:choose>
                    <c:when test="${res.team_type eq '0'}"><span class="">학원</span></c:when>
                  <c:when test="${res.team_type eq '1'}"><span class="">클럽</span></c:when>
                  <c:when test="${res.team_type eq '2'}"><span class="">유스</span></c:when>
                    </c:choose></td>
            ...
            <h4 style="display: inline-block">${result.league_name}</h4>
            <div class="others" style="float: right;">
              <!-- 
              <a class="btn-large default btn-tab" data-id="num1">전체</a>
              <a class="btn-large gray-o btn-tab" data-id="num2">3월</a>
              <a class="btn-large gray-o btn-tab" data-id="num3">4월</a>
              <a class="btn-large gray-o btn-tab" data-id="num4">5월</a>
              <a class="btn-large gray-o btn-tab" data-id="num5">9월</a>
              <a class="btn-large gray-o btn-tab" data-id="num6">10월</a>
               -->
            </div><br><br>

            <div class="scroll show-tab ac" id="num1">
              <table cellspacing="0" class="update fixed over">
                <colgroup>
						<col width="15%">
						<col width="7%">
                ...
                <tbody>
                	...
                	<c:forEach var="res" items="${teamLeagueEntertMatchList}" varStatus="status">
	                  <c:if test="${result.league_id eq res.league_id}">
		              	<tr>
		                  <td>${res.playdate}</td>
		                  <td>${res.ptime}</td>
            ...
            <div class="scroll show-tab" id="num2">
                <table cellspacing="0" class="update">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>고성군스포츠타운1구장</td>
                      <td class="tc">승(색상)</td>
                      <td><img src="../resources/img/logo/sample1.png" class="logo"><span class="">[학원]</span> 전북이리고</td>
                      <td>2</td>
                      <td>1</td>
                      <td><img src="../resources/img/logo/sample1.png" class="logo"><span class="">[유스]</span> 전주영생고</td>
                    </tr>
                  </tbody>
                </table>
            </div>	
          ...
          <div class="right w35 mobi">
            <div class="total-score-area">
              <div class="total-score">
                <div>
                  <strong>${teamCupAvgGoalMap.l_gf}</strong>
                  <p>리그 총 득점</p>
                </div>
              </div>
              <div class="total-score">
              ...
              <div class="total-score">
                <div>
                  <strong>${teamCupAvgGoalMap.l_ga}</strong>
                  <p>리그 총 실점</p>
                </div>
              </div>
              <div class="total-score">
            ...
            <h4>리그 순위 추이
                <c:if test="${isLeagueRankDuplicate}">
                    <br />
                    <span style="color: red;">[연동된 팀과 같은 연도의 데이터가 있을수 있습니다]</span>
                </c:if>
            </h4>
            ...
            <br>
            <h4>리그 평균 득/실점 추이
                <c:if test="${isLeagueAvgGoalDuplicate}">
                    <br />
                    <span style="color: red;">[연동된 팀과 같은 연도의 데이터가 있을수 있습니다]</span>
                </c:if>
            </h4>
