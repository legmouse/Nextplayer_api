...
<head>
    ...
    <script type="text/javascript">
        ...
        const searchList = () => {
            ...
            const showFlag = $("select[name='showFlag'] option:selected").val();
            ...
            newForm.attr('action', '/noticeList');
        ...
        const goDetail = (val) => {
            ...
            newForm.attr('name', 'newForm');
            newForm.attr('method', 'post');
            newForm.attr('action', '/noticeDetail');
...
<option>2014</option>
        ...
        <div class="round body">
            ...
            <div class="body-head">
                <a class="btn-large default" href="/noticeList">공지사항</a>
                <hr class="mt_10 mb_10">
                <div class="search">
                    <form name="frm" id="frm" method="post" action="/noticeList" onsubmit="return false;">
                        ...
                        <select class="w150" name="showFlag">
                  			<option value="" <c:if test="${param.showFlag eq '' or param.showFlag eq null}">selected</c:if>>메인노출여부</option>
                  			<option value="1" <c:if test="${param.showFlag eq '1'}">selected</c:if>>미노출</option>
                  			<option value="0" <c:if test="${param.showFlag eq '0'}">selected</c:if>>노출</option>
              			</select>
                                            ...
                                            <c:choose>
                                                <c:when test="${fn:length(result.files) > 0}">
                                                    <c:forEach var="i" items="${result.files}" varStatus="status">
                                                        <img src='/resources/img/icon/${i.fileExt}.gif' alt="" />
                                                    </c:forEach>
                                                </c:when>
                                                <c:otherwise>
                                                </c:otherwise>
